...
import { CQ9, JDB, JILI, MG, PG, EVO_Electronic } from "../../data/GameImg"
import { GameContext } from "../../context/GameContext"

// Use the imported game arrays
const EVO = EVO_Electronic
...
const GameIcons = {
    JILI: (color) => (
        <svg
                ...
                width="45"
                // height="20"
                fill={color === "#232626" ? "#232626" : "#B3BEC1"}
            >
                <symbol
                    id="icon-JILI"
                    viewBox="4 12 90 25"
                ...
                <use href="#icon-JILI" fill={color === "#232626" ? "#232626" : "#B3BEC1"} />
            </svg>
    ),
    PG: (color) => (
       <svg
            ...
            >
                ...
                <use href="#icon-PG" fill={color === "#232626" ? "#232626" : "#B3BEC1"} />
            </svg>
    ),
    CQ9: (color) => (
        <svg
    ...
    EVO: (color) => (
       <svg
            ...
            >
                ...
                {/* Reference the symbol here */}
                <use href="#icon-EVO_Electronic" fill={color === "#232626" ? "#232626" : "#B3BEC1"} />
            </svg>
    ),
}

const SlotsAllGames = () => {
    const [activeGame, setActiveGame] = useState("JILI")
...
const pathId = location.pathname.split('/').pop()
    ...
    useEffect(() => {
        // Check if a valid provider ID is in the URL path
        if (pathId && Object.keys(GAME_DATA).includes(pathId.toUpperCase())) {
            setActiveGame(pathId.toUpperCase())
        } 
        // Fallback to index parameter if present
        else if (index) {
            const gameMapping = {
                "0": "JILI",
                "1": "PG", 
            ...
            if (gameMapping[index]) {
                setActiveGame(gameMapping[index])
            }
        }
    }, [pathId, index])

    useEffect(() => {
        // Map URL index parameter to game providers
        const gameMapping = {
            "0": "JILI",
            "1": "PG",
            "2": "CQ9",
        ...
        if (index && gameMapping[index]) {
            setActiveGame(gameMapping[index])
        }
    }, [index])

    useEffect(() => {
        // Scroll active tab into view, but only within the tabs container
        if (activeGame && tabsContainerRef.current) {
            const tabElement = document.getElementById(`tab-${activeGame}`);
            if (tabElement) {
    ...
    zIndex: 1000, 
            ...
            >
                <Box sx={{ display: "inline-flex", padding: "0 13px" }}>
                    {Object.keys(GAME_DATA).map((gameKey) => {
                        const color = activeGame === gameKey ? "#232626" : "#B3BEC1"
                        ...
                        return (
                            <Button
                                ...
                                sx={{
                                    background: activeGame === gameKey ? "linear-gradient(180deg, #24ee89 0%, #FFB472 100%)" : "#323738",
                                    color: activeGame === gameKey ? "#232626" : "#B3BEC1",
                                    textTransform: "none",
                                    display: "flex",
                ...
                <Grid container spacing={0} sx={{ px: 1}}>
                    {gameItems.map((item, index) => (
                        <Grid
                            ...
                            onClick={() => handleApiClick(item.id, activeGame, "SLOT")}
                        >
                            <img
                                src={item.imgSrc}
                                alt={item.game}
