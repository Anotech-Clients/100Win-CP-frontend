...
const BettingRebate = () => {
  ...
  const tabData = [
    {
      ...
      selectedImage: "/assets/icons/all-selected.svg",
      label: "All",
      type: "All",
  ...
  const handleClaimRebate = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const { claim, transaction } = response.data.data.data;
        setSnackbar({
          open: true,
  ...
  useEffect(() => {
    fetchRebateData("All")
    if( !isLoading && pendingRebate > 0) {
      setIsDisable(false)
    }
  }, [pendingRebate, isLoading]);
      ...
      >
        ...
        <Box sx={{ mt: 1, mx: 1.5 }}>
          ...
          <Button
            ...
            onClick={() => navigate("/activity/betting-rebate-History")}
          >
            All history
          </Button>
          {/* <Box sx={{ mt: 1 }}>
