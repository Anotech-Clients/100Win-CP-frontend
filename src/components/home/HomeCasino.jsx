...
import { EVO_Video } from "../../data/GameImg";
const HomeCasino = ({onDetailClick = () => {}}) => {
  const Casino =EVO_Video

  const navigate = useNavigate();
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isAutoPlaying, setIsAutoPlaying] = useState(true);
  const itemsPerPage = 6;
  const totalPages = Math.ceil(Casino.length / itemsPerPage);
          ...
          {Array.from({ length: totalPages }).map((_, pageIndex) => (
            ...
            >
              {Casino.slice(
                ...
                // onClick={()=> handleApiClick(game.id, "EVO_PLAY", "CASINO")}
                >
                  <Box
                    sx={{
                      position: "relative",
